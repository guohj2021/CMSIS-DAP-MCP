//! Keil FLM flash algorithm parsing and probe-rs target YAML generation.
//!
//! Moved from the CLI crate so both the MCP server and CLI can generate
//! a target registry directly from an FLM file without requiring a
...
fn file_error(msg: impl Into<String>) -> McpError {
    McpError::new(ErrorCode::FileError, msg)
}
...
pub fn parse_flm(bytes: &[u8]) -> Result<FlmParse, McpError> {
...
pub fn generate_yaml(
    ...
    let load_address = sram_start
        ...
        .ok_or_else(|| McpError::new(ErrorCode::InvalidArgument, "sram range overflow"))?;
    if load_address
        ...
        .unwrap_or(true)
    {
        return Err(McpError::new(
            ...
            "SRAM region too small for the flash algorithm; increase --sram-size",
        ));
    }
...
/// Build a probe-rs target YAML and load it as a Registry, all in one step.
///
/// This is the convenience function used by both CLI `--flm` and MCP
/// `chip_generate` tool.
pub fn registry_from_flm(
    flm_path: &std::path::Path,
    name: Option<&str>,
    flash_start: u64,
    flash_size: u64,
    sram_start: u64,
    sram_size: u64,
    core: &str,
) -> Result<probe_rs::config::Registry, McpError> {
    let bytes = std::fs::read(flm_path)
        .map_err(|e| file_error(format!("failed to read {}: {e}", flm_path.display())))?;
    let parsed = parse_flm(&bytes)?;
    let chip_name_owned;
    let chip_name = match name {
        Some(n) => n,
        None => {
            chip_name_owned = flm_path
                .file_stem()
    ...
    let flash_end = flash_start
        .checked_add(flash_size)
        .ok_or_else(|| McpError::new(ErrorCode::InvalidArgument, "flash range overflow"))?;
    ...
    registry.add_target_family_from_yaml(&yaml).map_err(|e| {
        McpError::new(
            ErrorCode::FileError,
            format!("failed to parse generated yaml: {e}"),
        )
    })?;
    tracing::info!(
        "loaded target family from FLM {} as {chip_name}",
        flm_path.display()
    );
    Ok(registry)
}
