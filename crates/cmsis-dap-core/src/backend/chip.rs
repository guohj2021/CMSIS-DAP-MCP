//! Keil FLM -> probe-rs target YAML generation, shared by the MCP server and CLI.
//!
...
//!
//! This lives in `cmsis-dap-core` (not the CLI) so the MCP server can define a
//! custom/unknown chip at runtime via its `define_chip` tool without ever
//! shelling out to a standalone probe-rs CLI.
...
pub fn generate_yaml(
    ...
    let load_address = sram_start
        ...
        .ok_or_else(|| McpError::new(ErrorCode::InvalidArgument, "sram range overflows"))?;
    if load_address
...
/// Convenience: read an FLM file, derive the flash/SRAM end addresses, and
/// produce a probe-rs target YAML for the given chip `name` and `core`.
pub fn generate_target_yaml(
    flm_path: &Path,
    flash_start: u64,
    flash_size: u64,
    sram_start: u64,
    ...
    let bytes = std::fs::read(flm_path).map_err(|e| {
        McpError::new(
            ErrorCode::FileError,
            format!("failed to read {}: {e}", flm_path.display()),
        )
    })?;
    let parsed = parse_flm(&bytes)?;
    let flash_end = flash_start
        .checked_add(flash_size)
        .ok_or_else(|| McpError::new(ErrorCode::InvalidArgument, "flash range overflows"))?;
